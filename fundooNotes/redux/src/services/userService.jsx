...
export function createLabel(paramObject,loginToken){    
    console.log("Create label ===>",paramObject,loginToken);
    return axios.post(url+'/createLabel',paramObject,{
        headers:{token:loginToken}
    })
}


export function searchNote(paramValue,loginToken){    
    console.log("Search searchNote ===>",paramValue,loginToken);
    return axios.get(url+'/searchNote?search='+paramValue,{
        headers:{token:loginToken}
    })
}

export function deleteLabel(paramObject,loginToken){    
    console.log("update label ===>",paramObject,loginToken);
    return axios.post(url+'/deleteLabel',paramObject,{
        headers:{token:loginToken}
    })
}

export function getAllNoteL(param,loginToken){    
    console.log("LABEL NOTE SAME SERVICE in service===>",param);
    return axios.get(url+'/getAllNote?labelId='+param,{
        headers:{token:loginToken}
    })
}

// getAllNote?labelId=5dcd3e350a231868817e52fb